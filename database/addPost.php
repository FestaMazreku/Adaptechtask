...
if (isset($_POST['postsid']) && isset($_POST['userid']) && isset($_POST['title']) && isset($_POST['body'])) {
    ...
    if (!empty($postsid) && !empty($title) && !empty($body)) {
        ...
        if ($sql->affected_rows > 0) {
            ...
            $response['message'] = "Post is added successfully!";
            echo json_encode($response);
        } else {
            $response['success'] = false;
            $response['message'] = "Post is not added.";
            echo json_encode($response);
        }
